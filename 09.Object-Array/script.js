...
return anyNames.map((el) => el[0] || anyNames.sort((a,b) => a.localecCompare(b)) ).join("")
}
...
//1\\
console.log(users.filter((item) => item.uni==='UNEC' && item.surname.toLocaleLowerCase().startsWith("r")))

//2\\
console.log(users.filter((item)=> item.age==='22' && item.name.toLocaleLowerCase().startsWith('a')))

//3\\
console.log(users.map((item)=> item.gender==='male' ? `MR ${item.name}` :`Ms ${item.name}` ))
...
console.log(Math.random(letters).shuffle)

function random(anyLetters){
