...
let calcArea = arr3.map((el) =>
  el.region.length === 6 ? (sum += el.area) : sum
);
console.log(sum);

////2))
let calcArea2 = arr3.reduce((sum, prev)=> prev.region.length===6 ?sum+prev.area :sum,0)
console.log(calcArea2)
...
////2))
console.log(colors.sort((a,b)=> a.length-b.length ?1 :-1 && a.length==b.length ?a.localeCompare(b) :null))
